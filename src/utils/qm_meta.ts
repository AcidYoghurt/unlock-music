import { type IAudioMetadata, parseBlob as metaParseBlob } from 'music-metadata';
...
async function writeMetaToAudioFile(info: NewAudioMeta): Promise<Blob> {
  try {
    ...
    if (info.ext === 'mp3') {
      return new Blob([ToOwnedArrayBuffer(WriteMetaToMp3(buffer, newMeta, info.musicMeta))], { type: mime });
    } else if (info.ext === 'flac') {
      return new Blob([ToOwnedArrayBuffer(WriteMetaToFlac(buffer, newMeta, info.musicMeta))], { type: mime });
    } else {
