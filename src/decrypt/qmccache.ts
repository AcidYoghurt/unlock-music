...
export async function Decrypt(file: Blob, raw_filename: string, raw_ext: string): Promise<DecryptResult> {
  ...
  if (ext !== '' || newName.ext === 'mp3') {
    audioBlob = new Blob([ToOwnedArrayBuffer(musicDecoded)], { type: AudioMimeType[ext] });
  } else if (newName.ext in HandlerMap) {
    audioBlob = new Blob([ToOwnedArrayBuffer(musicDecoded)], { type: 'application/octet-stream' });
