import { KgmHeaderSize, SniffKgmEncryptedExt } from '@/decrypt/kgm_format';
import { DecryptResult, FileInfo } from '@/decrypt/entity';
import { GetArrayBuffer, SplitFilename } from '@/decrypt/utils';
...
export async function Decrypt(file: FileInfo, config: Record<string, any>): Promise<DecryptResult> {
  ...
  const hasKgmExtension = raw.ext === 'kgm' || raw.ext === 'kgma' || raw.ext === 'vpr';
  const detectedKgmExt = hasKgmExtension
    ? undefined
    : SniffKgmEncryptedExt(new Uint8Array(await GetArrayBuffer(file.raw.slice(0, KgmHeaderSize))));

  if (detectedKgmExt) {
    // KuGou Lite may append the decoded audio extension, e.g. "song.kgm.flac".
    // Route by the encrypted file header and remove the embedded KGM suffix from
    // the fallback filename used for downloads.
    const embeddedRaw = SplitFilename(raw.name);
    const rawFilename = ['kgm', 'kgma', 'vpr'].includes(embeddedRaw.ext) ? embeddedRaw.name : raw.name;
    rt_data = await (await import('@/decrypt/kgm')).Decrypt(file.raw, rawFilename, detectedKgmExt);
    rt_data.rawExt = detectedKgmExt;
    rt_data.rawFilename = rawFilename;
  } else
    switch (raw.ext) {
      case 'mg3d': // Migu Wav
      ...
      case 'kgma':
        ...
        break;
      case 'ofl_en':
        rt_data = await (await import('@/decrypt/joox')).Decrypt(file.raw, raw.name, raw.ext);
        break;
      case 'x2m':
      case 'x3m':
