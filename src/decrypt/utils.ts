...
/**
 * Copy a typed-array view into an ArrayBuffer owned by the current realm.
 * TypeScript 6 correctly rejects SharedArrayBuffer-backed views as BlobPart.
 */
export function ToOwnedArrayBuffer(data: ArrayBufferView<ArrayBufferLike>): ArrayBuffer {
...
export function GetCoverFromFile(metadata: IAudioMetadata): string {
  if (metadata.common?.picture && metadata.common.picture.length > 0) {
    return URL.createObjectURL(
      new Blob([ToOwnedArrayBuffer(metadata.common.picture[0].data)], { type: metadata.common.picture[0].format }),
    );
  }
  return '';
}
...
function normalizeId3FrameValue(value: unknown): string | string[] | object | undefined {
...
export function WriteMetaToMp3(audioData: Buffer, info: IMusicMeta, original: IAudioMetadata) {
  ...
  frames.forEach((frame) => {
    if (frame.id !== 'TPE1' && frame.id !== 'TIT2' && frame.id !== 'TALB') {
      try {
        const value = normalizeId3FrameValue(frame.value);
        if (value !== undefined) writer.setFrame(frame.id, value);
      } catch (e) {
