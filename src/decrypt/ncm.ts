...
class NcmDecrypt {
    ...
    if (this.oriMeta.albumPic)
      try {
        ...
        while (this.image && this.image.buffer.byteLength >= 1 << 24) {
          ...
          this.image.buffer = ToOwnedArrayBuffer(await img.getBufferAsync('image/jpeg'));
        }
      } catch (e) {
  ...
  async _writeMeta() {
    ...
    if (!this.blob) this.blob = new Blob([ToOwnedArrayBuffer(this.audio)], { type: this.mime });
    ...
    if (shouldWrite || this.newMeta.picture) {
      ...
      this.blob = new Blob([ToOwnedArrayBuffer(this.audio)], { type: this.mime });
    }
  }
